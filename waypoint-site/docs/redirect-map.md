...
### A2b. Auth continuity (prep during Phase A; BLOCKS the Phase B domain swap)

The app requires login, and auth is host-sensitive in two ways:

1. **Redirect allow-lists.** Before any user lands on `app.waypointchild.com`:
   - Supabase Auth → URL Configuration: add `https://app.waypointchild.com` (and its auth callback paths) to the additional redirect URLs; plan to update Site URL to the app host at swap time. **[verify exact fields at execution]**
   - Google OAuth: Supabase's Google provider normally uses the Supabase-hosted callback (`https://<project>.supabase.co/auth/v1/callback`), so the Google Cloud client *may* need no change — **verify at execution**; if the app passes its own `redirectTo`, the new host must be allow-listed in both Supabase and any provider config.
   - Grep `waypoint-app` for hardcoded `www.waypointchild.com` / `waypointchild.com` URLs (deep links, share links, email templates) and repoint them to the app host. **Grep done 2026-09-06:** `src/lib/appLinks.ts` sets `DEFAULT_WEB_ORIGIN = 'https://waypointchild.com'` — invite links (`/join?token=…`) and shared actions (`/actions/:id`) use the APEX. Covered two ways: `waypoint-site/vercel.json` now carries host-agnostic permanent redirects for `/join` and `/actions/*` → the app host (so links generated before the swap keep working forever), and `DEFAULT_WEB_ORIGIN` flips to `https://app.waypointchild.com` in the swap-window PR (keeping already-sent links valid via the redirects).
2. **Sessions do not follow the host.** Supabase sessions are stored per-origin (localStorage), so existing signed-in users on `www` will be **signed out once** when they arrive at `app.waypointchild.com` and must log in again. This is expected and acceptable — but do the swap in a quiet window, and expect a small support blip. The D3 first-touch write-once rule is unaffected (re-login is not a new first touch).

Verification before Phase B: complete one full login (Google + email paths) on `https://app.waypointchild.com` while `www` still serves the app.
