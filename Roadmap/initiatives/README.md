...
| 008 | Chabot local tools — hyper-local pilot: Family Navigator 1-pager, parent Google Group, plus an owner-directed community hub prototype (Discussions + Wisdom, flag-only moderation, AI search) | Open — Navigator awaiting owner fill-in + fact re-verify; hub prototype published, no backend; family-facing, so nothing auto-ships | — | [intent](008-chabot-local-tools/intent.md) · [recommendations](008-chabot-local-tools/recommendations.md) · [navigator draft](008-chabot-local-tools/navigator-draft.md) · [hub plan](008-chabot-local-tools/hub-plan.md) |
