...
| D2-R | **Owner decision, 2026-09-09 (chat session), REVOKES part of D2.** The published-requires-review-block refine (`content.config.ts`, all of `guides`/`answers`/`letters`/`regionalCenters`/`research`) is removed site-wide. Trigger: 21 RC pages + 4 IEP/IPP guides were ready to publish except for a reviewer, and the owner chose to publish without one rather than wait — and explicitly asked that no reviewer name (including his own) be attached to these pages, unlike the 2026-09-07 "Publish, I'm the reviewer" precedent on `/start/autism/` etc. Net effect: `review: null` no longer blocks `published`. Still standing, unchanged: the D11 RC `verifiedAsOf` gate, and the `[TBC]` ratchet (`check-tbc.mjs`) — those facts must be resolved or honestly disclosed on-page, not hidden. See execution log below for what published under this decision. |
