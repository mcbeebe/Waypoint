...
## B. App icon + web favicon — the Waypoint marker ✅ DONE (compact pin)

> **Done 2026-09-01.** Owner chose the **compact pin** (no route line) for the
> app icon / favicon — a route line is illegible at icon sizes. Generated by
> rendering the pin geometry with the pre-installed Chromium
> (`headless_shell --screenshot`, no rsvg/magick needed) at 1024/512/48 and
> writing `assets/{icon,adaptive-icon,favicon,splash-icon}.png`. `app.json`
> splash + adaptiveIcon `backgroundColor` warmed `#1B2A4A` → `#F5F1E9`. The
> full route mark is used in-app (Home header, Journey hero) via `<Brandmark
> route>`, not on the OS icon. Original SVG recipe kept below for reference.
