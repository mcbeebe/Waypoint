...
const DRAFT_PROMPTS: Record<string, string> = {
  ...
  ipp_need_request: `Draft a collaborative written request to the family's Regional Center Service Coordinator asking to add a specific identified NEED to the child's IPP (Individual Program Plan), so that a support meeting that need can be funded. Key legal anchor: the IPP is built around the consumer's needs and the planning team considers the family's needs as they relate to the consumer (W&I Code §4646.5), and the Regional Center secures the services the IPP identifies (W&I Code §4648(a)) — so getting the need written into the plan is the move, not asking for a reimbursement after the fact. The parent's message names the specific need (for example a sibling's adjustment, caregiver relief, community integration, or building the parent's own capacity) and the support they'd like to look at. The letter should: 1) name the specific need in the family's own words, 2) ask that it be recorded as an identified need in the IPP, 3) ask to look together at the support that would meet it — by name where the parent gave one, 4) request written confirmation of what will be added. Do NOT request a formal IPP review meeting in this letter — that starts a separate 30-day statutory clock and belongs to a different request; keep this a collaborative ask to record the need and look at the support together. This is a COLLABORATIVE ask, not a demand — it stays warm and partnership-framed at every tone; even at strong tone it firms up only to "we'd like this documented and confirmed in writing," never adversarial, and it invents no statutory deadline (there is none for this request). Use [BRACKETS] for information the parent needs to fill in.`,
