...
export default function PricingScreen() {
  ...
  const { isPremium, sponsorLabel, refetch } = useEntitlement(family?.id);
  const { showToast } = useToast();
  const [claiming, setClaiming] = React.useState(false);

  /**
   * The second door (060). One tap, no form, no proof, no follow-up
   * question — the server-side check is only "is this your family."
   *
   * Families whose child is on SSI never reach this: the trigger grants
   * before they see a price. This exists for everyone SSI misses, which is
   * a lot of people — most childhood SSI claims are denied on medical
   * criteria, parental deeming disqualifies many low-income working
   * families, and applications sit in backlog for months.
   */
  const claimFree = async () => {
    setClaiming(true);
    try {
      const { error } = await supabase.rpc('claim_community_entitlement', { p_reason: null });
      if (error) throw error;
      await refetch();
      showToast('Done — Waypoint is free for your family from now on.', 'success');
    } catch {
      // Never leave them stuck at a paywall because our RPC failed.
      showToast('Something went wrong on our end. Email privacy@waypointchild.com and we will turn it on for you.', 'error');
    } finally {
      setClaiming(false);
    }
  };
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {!isPremium ? (
        <View style={styles.affordCard}>
          <Text style={styles.affordTitle}>Money tight right now?</Text>
          <Text style={styles.affordBody}>
            Then Waypoint is free for you. Not a trial, not a discount — the whole thing,
            for as long as you need it. You already prove your situation to the regional
            center, the district, the county and Social Security. You don't have to prove
            it to us. No forms, no paperwork, no follow-up questions.
          </Text>
          <Pressable
            style={[styles.affordCta, claiming && styles.affordCtaBusy]}
            onPress={claimFree}
            disabled={claiming}
            accessibilityRole="button"
            accessibilityLabel="Make Waypoint free for my family"
          >
            <Text style={styles.affordCtaText}>
              {claiming ? 'One moment…' : 'Make Waypoint free for my family'}
            </Text>
          </Pressable>
          <Text style={styles.affordFootnote}>
            Funded by the families who do pay. Take it — that's what it's for.
          </Text>
        </View>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  // Deliberately NOT styled as a lesser option tucked in the fine print. A
  // parent who needs this should find it as easily as the buy button.
  affordCard: {
    marginTop: spacing.base,
    backgroundColor: colors.white,
    borderWidth: 1,
  ...
  affordTitle: { fontSize: fonts.sizes.lg, fontWeight: fonts.weights.bold, color: colors.navy },
  affordBody: {
    marginTop: spacing.sm,
  ...
  affordCtaBusy: { opacity: 0.6 },
  affordCtaText: { color: colors.teal, fontSize: fonts.sizes.md, fontWeight: fonts.weights.bold },
  affordFootnote: {
    marginTop: spacing.sm,
    fontSize: fonts.sizes.sm,
    color: colors.mid,
