...
> - **W3 — evidence & decision.** Operational phase: real families through the pipeline, price interviews, funnel verdict, go/no-go memo. The Scorecard reports the four kill-criteria metrics and exports the full evidence readout (funnel by source/language, hours-by-stage, baseline deltas, computed verdicts). All E3 Premium gates are wired (docs sharing, paper-trail export, multi-child; free promises stay free) and the funnel is trilingual (EN/ES/VI, parity-guarded). Remaining code work is demand-driven: RC export formats await RCEB pilot feedback; Stripe hookup is the open ops item.
> - **Resource Stack build-out (Aug 26, 2026) — code complete, all four phases** (Roadmap/Resource-Stack-Journey-Plan.md): sdpJourney + resourceStack content modules with D-2026-SDP-002 compliance (two-part SCDD orientation, 4-item hand-off) rippled through letters/process map/sent moments; SDP Journey stepper (Concept B) with self-reported position; Home stack insight + Medi-Cal-deeming unlock sheet (Concept C) with new medi_cal_deeming letter; full Resource Stack view on Journey (Concept A). Trilingual + provenance-registered throughout. *Open (ops):* apply migration 043 in the Supabase SQL Editor.
